/*
 * TGSLN.C - test of graphics library
 *
 * Source Version: 3.0
...
/* set up the hard copy device */
    if (use_color)
       PS_dev = PG_make_device("PS", "COLOR", "tgsln 2.0 2.0");
    else
       PS_dev = PG_make_device("PS", "MONOCHROME", "tgsln 2.0 2.0");
    PG_turn_data_id(PS_dev, ON);
    PG_open_device(PS_dev, 0.0, 0.0, 0.0, 0.0);

/* set up the hard copy device */
    if (use_color)
       CGM_dev = PG_make_device("CGM", "COLOR", "tgsln");
    else
       CGM_dev = PG_make_device("CGM", "MONOCHROME", "tgsln");
