...
; FIND-INDEX-RANGE-EXPR - scan the string NAME for patterns like
;                       - .*[d]*:[d]*.*
;                       - where .* means match zero or more characters
;                       - and [d]* means match one or more digits
...
(define (find-index-range-expr name)
    (let* ((nm (sprintf name))
	   (sa (strtok nm ":"))
           (sinc (strchr nm ":"))
	   (sb (if (string? sinc)
...
; PRINT-INDEX-RANGE - reduce non-terminal index range to loop over
;                   - single values and recurse on the printing of
;                   - the reduced expression

(define (print-index-range file pa imn i imx pb inc tm indl)
   (if (and (<= imn i) (<= i imx))
       (let* ((name (sprintf "%s%d%s%s"
			     pa i
			     (if pb pb "")
			     (if tm tm ""))))
	     (print-file-variable file name indl)
	     (print-index-range file pa imn (+ i inc) imx pb inc tm))))

;--------------------------------------------------------------------------
;--------------------------------------------------------------------------

; PRINT-FILE-VARIABLE - recursively handle the printing of file variable
;                     - specification with non-terminal index ranges
;                     - which PDBLib will not handle

(define (print-file-variable file name indl)
...
(define (print-find-expr msg var expr)
...
(define (print-index-only file var lst n)
    (if lst
	(let* ((tuple (index->tuple-string file var
	      ...
	      (if (= (remainder n 4) 0)
		  (printf nil "\n"))
	      (print-index-only file var (cdr lst) (+ n 1)))))

;--------------------------------------------------------------------------
;--------------------------------------------------------------------------

(define (print-index file var expr ind n)
    (if lst
	(cond ((eqv? find-mode-flag index+value)
	      ...
	      (else
	       ...
	       (printf nil "\n")
	       (print-index-only file var (pm-array->list ind) n)
	       (printf nil "\n")))))
...
(define (find-and var arr prd val rst indl)
    (find-aux var arr rst
	      (find-index arr prd val indl)))

;--------------------------------------------------------------------------
;--------------------------------------------------------------------------

(define (find-or var arr prd val rst indl)
    (let* ((aid (find-index arr prd val indl))
	   (bid (find-aux var arr rst))
...
(define (find-aux var arr expr indl)
	      ...
	      (cond ((= nargs 2)
		    ...
		    (else
		     (let* ((fnc (list-ref expr 0))
		       ...
		       (cond ((eqv? cnj 'and)
			      (find-and var arr prd val rst indl))
			     ((eqv? cnj 'or)
			      (find-or var arr prd val rst indl))
			     (else
			      (printf nil
...
(define-macro (find var . expr)
    ...
    (let* ((arr (pdbdata->pm-array (read-pdbdata current-file var)))
	   ...
	   (oid (find-aux var arr expr nil)))

          (if (interactive?)
	      (begin (print-index current-file var expr oid 1)
		     (printf nil "\n")))
	  oid))
