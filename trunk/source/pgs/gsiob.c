...
static void PG_align_interface_object(PG_interface_object *iob, int xo, int yo)
   {int i, n;
    ...
    ch = SC_array_array(iob->children, 0);
    for (i = 0; i < n; i++)
        {crv = ch[i]->curve;
...
static void _PG_draw_variable_object(PG_interface_object *iob)
   {int i, nch, ityp, match, isa;
	...
	chn = SC_array_array(iob->children, 0);
...
static void _PG_draw_slider_button(PG_interface_object *iob)
   {int i, nv, nch;
    ...
    chn  = SC_array_array(piob->children, 0);
    ...
    SC_array_unarray(piob->children, 0);
...
/* copy the children */
    ...
    sch = SC_array_array(iob->children, 0);
