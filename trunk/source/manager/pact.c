/*
 * PACT.C - wrapper to control make to PACT's needs
 *
 * Source Version: 3.0
...
static void report_cl(char *q)
   {
    ...
    if (strcmp(q, "-incpath") == 0)
       {report("MDInc", FALSE, TRUE, FALSE);
	report("MDGInc", FALSE, TRUE, TRUE);}

    else if (strcmp(q, "-link") == 0)
       {report("MDGLib", FALSE, TRUE, FALSE);
	report("DPLib", FALSE, TRUE, FALSE);
...
static int invoke_make(char *cmd, int nc, int c, char **v)
   {int i, st, na, ns;
    char s[MAXLINE];
    char *log;
...
int main(int c, char **v)
   {int i, status, keep, ok, nproc, na, litrl, mc;
    long tmm;
    char mkfile[MAXLINE];
    char *pmname, *mv[100];
