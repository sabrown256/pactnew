...
static int
 _PP_get_sequence_descr(PP_file *fileinfo, PyObject *obj,
			int nd, PP_getdescr *seqinfo);
...
static PP_class_descr *PP_inquire_class(PP_file *fileinfo, PyTypeObject *cls)
   {PP_class_descr *cdescr;

    cdescr = (PP_class_descr *) SC_hasharr_def_lookup(fileinfo->class_map,
						      (void *) cls);
...
PP_descr *PP_make_descr(PP_types typecode, char *type, long bpi)
   {

    return(_PP_mk_descr(typecode, bpi, CSTRSAVE(type), NULL, NULL));}
...
PP_descr *_PP_mk_descr(PP_types typecode, long bpi,
		       char *type, dimdes *dims, void *data)
   {PP_descr *descr;
...
PP_type_entry *PP_make_type_entry(PP_types typecode, int sequence,
				  PP_descr *descr, PyTypeObject *otype,
				  PP_pack_func pack, PP_unpack_func unpack,
...
PP_type_entry *PP_inquire_type(PP_file *fileinfo, char *ctype)
   {PP_type_entry *entry;
    ...
    entry = (PP_type_entry *) SC_hasharr_def_lookup(fileinfo->type_map,
						    ctype);
...
void PP_register_object(PP_file *fileinfo, PP_type_entry *entry)
   {char *nm;
    ...
    if (entry->cdescr != NULL)
       nm = (char *) entry->cdescr->cls->tp_name;
    else if (entry->obtyp != NULL)
       nm = (char *) entry->obtyp->tp_name;
    else
       nm = "unknown";

    ha = fileinfo->object_map;
    SC_hasharr_install(ha, nm, entry, PY_OBJECT_MAP_S, 3, -1);
...
PP_class_descr *PP_make_class_descr(PyTypeObject *cls, char *type,
...
PP_type_entry *PP_inquire_object(PP_file *fileinfo, PyObject *obj)
   {char *nm;
    ...
    for (tp = PY_TYPE(obj); tp != NULL; tp = tp->tp_base)
        {nm    = (char *) tp->tp_name;
	 entry = (PP_type_entry *) SC_hasharr_def_lookup(ha, nm);
	 if (entry != NULL)
	    break;};
...
/* type hash table */
    fileinfo->type_map   = SC_make_hasharr(HSZSMALL, NODOC,
					   SC_HA_NAME_KEY, 0);
    fileinfo->object_map = SC_make_hasharr(HSZSMALL, NODOC,
					   SC_HA_ADDR_KEY, 0);
    fileinfo->class_map  = SC_make_hasharr(HSZSMALL, NODOC,
					   SC_HA_ADDR_KEY, 0);
...
/* prototype reminders:
 *
 * PP_make_descr(<typecode>, <type>, <bpi>)
 * PP_make_type_entry(<typecode>, <sequence>, <descr>,
 *                    <otype>, <pack>, <unpack>, <get_descr>)
 */
...
/* register long type */
    ...
    entry = PP_make_type_entry(PP_LONG_I, FALSE, descr, &PY_INT_TYPE,
			       ...
			       _PP_get_unpack_func(PP_LONG_I),
			       NULL);

    PP_register_type(fileinfo, entry);
    PP_register_object(fileinfo, entry);
    
/* register PyLong_Type, 'long' type maps to PY_INT_TYPE */
    entry = PP_make_type_entry(PP_LONG_I, FALSE, descr, &PyLong_Type,
			       _PP_get_pack_func(PP_LONG_I),
			       _PP_get_unpack_func(PP_LONG_I),
			       NULL);
...
/* register float type */
    ...
    entry = PP_make_type_entry(PP_FLOAT_I, FALSE, descr, &PyFloat_Type,
			       ...
			       _PP_get_unpack_func(PP_FLOAT_I),
			       NULL);
...
/* register double type */
    ...
    entry = PP_make_type_entry(PP_DOUBLE_I, FALSE, descr, &PyFloat_Type,
			       ...
			       _PP_get_unpack_func(PP_DOUBLE_I),
			       NULL);
...
#else
    entry = PP_make_type_entry(PP_INSTANCE_I, FALSE, NULL, &PyInstance_Type,
			       NULL, NULL, _PP_get_instance_descr);
#endif
...
/* PP_init_type_map_instance - Install into type_map.
 *                           - Install into class_map.
 *                           - Install into object_map.
 */

void PP_init_type_map_instance(PP_file *fileinfo, PP_class_descr *cdescr)
   {PP_type_entry *entry;

    entry = PP_make_type_entry(PP_INSTANCE_I, FALSE, cdescr->descr,
			       NULL, NULL, NULL, NULL);
...
void PP_init_type_map(void)
   {
    ...
    PY_OBJECT_MAP_S = CSTRSAVE("PP_type_entry");
    SC_permanent(PY_OBJECT_MAP_S);
...
int PP_copy_obj_to_mem(PyObject *obj, PP_file *fileinfo,
		       PP_type_entry *entry, long nitems,
                       void *vr, dimdes *dims)
   {int mitems, err;
...
/* lookup the item we have to find out how to get what we want */
	   {objentry = PP_inquire_object(fileinfo, obj);
		   ...
		   {int mitems2 = 0;
			 ...
			 if (next == NULL)
                            return(-1);
			 mitems = PP_copy_obj_to_mem(next, fileinfo,
						     entry, dims->number,
						     vr, dims->next);
			 if (mitems == -1)
                            break;
			 vr = (void *) (((char *) vr) +
					entry->descr->bpi*mitems);
			 mitems2 += mitems;};
...
PyObject *PP_copy_mem_to_obj(PDBfile *file, void *vr, dimdes *dims,
                             long nitems, char *type, PP_type_entry *entry,
                             PP_form *form, int sequence)
   {PyObject *obj;
...
int PP_make_data(PyObject *obj, PP_file *fileinfo, char *type,
   ...
   {int ierr;
	...
	if (ierr < 0)
	   {_PP_rel_syment(fileinfo->file->host_chart, pv, nitems, type);
	    CFREE(pv);};
...
PP_descr *_PP_get_object_descr(PP_file *fileinfo, PyObject *obj,
			       PP_type_entry *entry)
   {PP_descr *descr;
...
/* _PP_get_sequence_descr_work - find type and shape for a
 *                             - member of a sequence.
 *  See if object is a of a registered type.
 *  Nested sequence
 */

static int _PP_get_sequence_descr_work(PP_file *fileinfo, PyObject *obj,
				       int nd, PP_getdescr *seqinfo)
   {int rv;
    PP_descr *descr;
    PP_type_entry *entry;
...
static int _PP_get_sequence_descr(PP_file *fileinfo, PyObject *obj,
				  int nd,  PP_getdescr *seqinfo)
   {int rv;
    Py_ssize_t i, nitems;
    PyObject *fast, *next;
    ...
    if (nd >= MAXDIM)
       {PP_error_set_user(NULL,
			  "Sequences nested too deeply, max %d", MAXDIM);
        return(-1);};
...
/* remove last dimension since it represents the members */
		       {dprev = descr->dims;
...
/* #define'd floating point constants */
    ne += (PyModule_AddObject(m, "SMALL",
			      PyFloat_FromDouble((double) SMALL)) < 0);
    ne += (PyModule_AddObject(m, "HUGE",
			      PyFloat_FromDouble((double) HUGE)) < 0);
    ne += (PyModule_AddObject(m, "TOLERANCE",
			      PyFloat_FromDouble((double) TOLERANCE)) < 0);
