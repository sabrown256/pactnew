;
; VCEL.SCM - multi-d table visualization routines
;          - Lee Minner  100306
;
; Source Version: 4.0
...
; TPLOT2D - generate a 2-d plot of the mapping created referencing
;         - all the points of the current table

(define-macro (tplot2d tlabel)
    "TPLOT2D - Make a 2-D plot of the mapping created using all the rows and columns of the current table.
               The default viewport rendering is set to fill-poly.
     Usage: tplot2d <label>
     Example: tplot2d plotlabel"
    (let* ((d1 (car tdims))
           (d2 (cadr tdims))
           (tnpts (* d1 d2))
           (stlabel (print-name tlabel))
           (mp (table->pm-mapping stlabel (list stlabel nil) (list stlabel (list 1) (list 0 tnpts 1)) zone)))
          (set! labelsave tlabel)
          (dl)
          (if current-window
              (clear-ann))
          (display-mapping* mp)
...
;          -  original label passed to tplot2d
...
(define (help-vcel)
    ...
    (printf nil "    tplot2d         - Plot the current table as a poly-fill rendering\n")
    ...
    (printf nil "    tdomv           - Domv for vcel\n")
