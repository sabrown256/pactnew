...
/* floating point types */
	 if (strncmp(typ, SC_FLOAT_S, 5) == 0)
	    {dv = *(float *) pc->cdr;
	     o  = SS_mk_float(dv);
	     rv = SS_mk_cons(o, rv);}

	 else if (strncmp(typ, SC_DOUBLE_S, 6) == 0)
	    {dv = *(double *) pc->cdr;
	     o  = SS_mk_float(dv);
	     rv = SS_mk_cons(o, rv);}

	 else if (strncmp(typ, SC_LONG_DOUBLE_S, 6) == 0)
...
/* fixed point types */
	 else if (strncmp(typ, SC_SHORT_S, 5) == 0)
	    {lv = *(short *) pc->cdr;
	     o  = SS_mk_integer(lv);
	     rv = SS_mk_cons(o, rv);}

	 else if (strncmp(typ, SC_INT_S, 3) == 0)
	    {lv = *(int *) pc->cdr;
	     o  = SS_mk_integer(lv);
	     rv = SS_mk_cons(o, rv);}

	 else if (strncmp(typ, SC_LONG_S, 4) == 0)
	    {lv = *(long *) pc->cdr;
	     o  = SS_mk_integer(lv);
	     rv = SS_mk_cons(o, rv);}

	 else if (strncmp(typ, SC_LONG_LONG_S, 9) == 0)
	    {lv = *(long long *) pc->cdr;
	     o  = SS_mk_integer(lv);
	     rv = SS_mk_cons(o, rv);}

	 else if (strncmp(typ, SC_CHAR_S, 4) == 0)
